...
class RandomGaussianMixture(Model_generator):
    """
    Mixture of randomly correlated gaussians with uniform prior.
    """
    def __init__(self, ndim=2, nmodes=2, prior_size_in_std=8., random_mean_in_std=1,
                 input_params=None, seed=None):
        self.ndim = ndim
        self.nmodes = nmodes
        self.prior_size_in_std = prior_size_in_std
        self.random_mean_in_std = random_mean_in_std
        ...
        # Different weights
        # self.weights = np.array([(i + 1) * 1 / (self.nmodes + 1)
        #                          for i in range(self.nmodes, 2 * self.nmodes)])
        # self.weights /= np.sum(self.weights)
        # Equal weights
        self.weights = [1 / self.nmodes for _ in range(self.nmodes)]

    def redraw(self):
        self.std = np.array([self.rng.uniform(size=self.ndim) for _ in range(self.nmodes)])
        eigs = [self.rng.uniform(size=self.ndim) for _ in range(self.nmodes)]
        eigs = [eigs_i / np.sum(eigs_i) * self.ndim for eigs_i in eigs]
        corr = [random_correlation.rvs(eigs_i, random_state=self.rng)
                if self.ndim > 1 else [[1]] for eigs_i in eigs]
        self.cov = [np.multiply(np.outer(self.std[i], self.std[i]), corr[i])
                    for i in range(self.nmodes)]
        self.bounds = np.array(
            [(-self.prior_size_in_std * max(self.std[:, i]),
              self.prior_size_in_std * max(self.std[:, i]))
             for i in range(self.ndim)])
        self.mean = np.array(
            [self.rng.uniform(
                low=self.bounds[i][0], high=self.bounds[i][1], size=self.nmodes)
    ...
    def get_model(self):
        if self.mean is None:
            self.redraw()
        def gaussian_mixture(**kwargs):
            X = [kwargs[p] for p in self.input_params]
            return np.log(np.sum([w * rv.pdf(X) for w, rv in zip(self.weights, self.rv)]))
        info = {"likelihood": {"gaussian_mixture": {
            "external":gaussian_mixture, "input_params": self.input_params}}}
        info["params"] = {}
        for k, p in enumerate(self.input_params):
            info["params"][p] = {"prior": {"min": self.bounds[k][0], "max": self.bounds[k][1]}}
        print(info)
